...
public class UserCacheDaoImpl<K, V extends UserWithInfo> implements CacheDao<K, V> {

    private static final String SYNTHETIC_REQUEST = "SELECT 1 FROM DUAL";
    private static final Logger log = org.slf4j.LoggerFactory.getLogger( UserCacheDaoImpl.class );

    private final IgniteCache<K, V> cache;
    private final Ignite ignite;

    public UserCacheDaoImpl( IgniteCache<K, V> cache, Ignite ignite ) {
        this.cache = cache;
        this.ignite = ignite;
    }
    ...
    public void save( V value ) {
        if ( !cache.putIfAbsent( ( K )value.id, value ) ) {
            log.info( "Value with key = " + value.id + " already exist" );
        }
    }

    @Override
    public void update( V value ) {
        cache.put( ( K )value.id, value );
    }
